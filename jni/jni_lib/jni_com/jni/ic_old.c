...
//jbyteArray readiccard(JNIEnv* env,int fds)
jstring readiccard(JNIEnv* env,int fds)
{
	...
	unsigned Authentication_flag = 0;
	char * rd_id = malloc(4);
	...
	while (1) {
		...
		if(loops ==0)
			break;
		__android_log_print(ANDROID_LOG_INFO, "Acanoe", "start read ic card");
		if(!(lll=THM_MFindCard(rd_id)))
		{
			__android_log_print(ANDROID_LOG_INFO, "Acanoe", "THM_MFind");
			for(i=0;i < 4;i++)
			{
				__android_log_print(ANDROID_LOG_INFO, "Acanoe", "readicicard %d",rd_id[i]);
				sprintf(nu, "%s%02x",nu,rd_id[i]);
			}
